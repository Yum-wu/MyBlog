    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
      ...
      <div className="text-sm text-neutral-500 dark:text-neutral-400">
        {totalItems !== undefined && pageSize && (
          <span>
            {t("pagination.total")} <span className="font-medium text-neutral-700 dark:text-neutral-300">{totalItems}</span> {t("pagination.items")}
          </span>
        )}
        <span>
          {t("pagination.currentPage")} <span className="font-medium text-neutral-700 dark:text-neutral-300">{current}</span> / {total} {t("pagination.page")}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        {showJumper && total > 5 && (
          <div className="ml-2 flex items-center gap-1.5 text-sm text-neutral-500 dark:text-neutral-400">
            <span className="hidden sm:inline">{t("pagination.jumpTo")}</span>
            <input
              type="number"
              min={1}
              max={total}
            ...
            />
            <span className="hidden sm:inline">{t("pagination.go")}</span>
            <button
              type="button"
              onClick={handleJump}
